...
uint32_t BEEP_Send( uint8_t fport, uint8_t * payload, uint8_t payload_lenght )
{
    ...
    if( LoRaMacQueryTxPossible( payload_lenght, &txInfo ) != LORAMAC_STATUS_OK )
    {
        // Send empty frame in order to flush MAC commands
        mcpsReq.Type = MCPS_UNCONFIRMED;
        mcpsReq.Req.Unconfirmed.fBuffer		= NULL;
    ...
    NRF_LOG_INFO( "FPORT: %u, Trigger: %s, STATUS      : %s", fport, BeepMessagestatus_str(fport), getMacStatusStringPointer(status) );
    ...
    switch( mlmeConfirm->MlmeRequest )
    {
        case MLME_JOIN:
        {
            ...
            if( mlmeConfirm->Status == LORAMAC_EVENT_INFO_STATUS_OK )
            {
                ...
                // Perform a callback to notify the main that OTAA has been succesfully completed
                BEEP_protocol_s callback;
                callback.command = LORAWAN_OTAA_COMPLETE;
                if(main_callback != NULL)
                {
                    main_callback(INTERNAL_SOURCE, &callback);
                }
